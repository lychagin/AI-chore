...
# Скрипт для управления MCP DGraph сервером
# Использование: ./mcp-server-manager.sh {start|stop|status|restart}
...
start_mcp_server() {
    echo "🚀 Запуск MCP DGraph сервера..."
    ...
    if [ -f "$MCP_SERVER_PATH" ]; then
        echo "✅ MCP DGraph сервер готов к работе"
        echo "📝 Путь к серверу: $MCP_SERVER_PATH"
        echo "💡 MCP сервер запустится Cursor-ом автоматически при использовании MCP инструментов"
    else
        echo "❌ MCP сервер не найден: $MCP_SERVER_PATH"
        echo "💡 Выполните: npm run mcp:build"
        return 1
...
stop_mcp_server() {
    echo "🛑 Остановка MCP DGraph сервера..."
...
case "$1" in
    ...
    *)
        ...
        echo ""
        echo "Команды:"
        echo "  start   - Запустить MCP DGraph сервер"
        echo "  stop    - Остановить MCP DGraph сервер"
        echo "  status  - Проверить статус MCP DGraph сервера"
        echo "  restart - Перезапустить MCP DGraph сервер"
        exit 1
        ;;
esac
